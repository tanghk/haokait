...
- (void)loadmyBankCardData:(id)result{
    ...
    if ([code intValue] ==200) {
       ...
       // BOOL isDefaultCard = [dataDic[@"isDefaultCard"]boolValue];
        ...
        NSDictionary *defaultCard = dataDic[@"defaultBankcard"];

        /*
         *默认卡
         */
        
        _cardNumber = [NSString stringWithFormat:@"%@",defaultCard[@"card"]];
        NSString *cardNumb = [_cardNumber substringFromIndex:_cardNumber.length-4];
        _bankCardNumber = [NSString stringWithFormat:@"%@",defaultCard[@"bankcard"]];
        
        [_bankLogo sd_setImageWithURL:[NSURL URLWithString:defaultCard[@"logo"]]];
        _bankName.text = [NSString stringWithFormat:@"%@(尾号%@)",defaultCard[@"bankname"],cardNumb];

        /*
         *银行卡名字
         */
        _bankNameValue =[NSString stringWithFormat:@"%@",defaultCard[@"bankname"]];
        ...
        NSMutableArray *listData = dataDic[@"list"];
        for (NSDictionary *bankInfo in listData) {
            
            WithdrawModel *model = [[WithdrawModel alloc]initWithDataDic:bankInfo];
            
            [self.data addObject:model];
            
            self.withdraw.data = self.data;
        }
...
//                if (_is_holiday ==NO&&_tomrrow_Is_Holiday ==YES) {
//                    
//                    NSDecimalNumberHandler*roundUp = [NSDecimalNumberHandler
//                                                      
//                                                      decimalNumberHandlerWithRoundingMode:NSRoundPlain
...
//                    _withdrawFeeValue = [NSString stringWithFormat:@"%.2f",([total floatValue]*[_holiday_days intValue] )+2];//手续费
//                    
//                    _realityAmountValue =[NSString stringWithFormat:@"%.2f",[_amount_entered floatValue]- [_withdrawFeeValue floatValue]];
//                    _realityAmount.text =[NSString stringWithFormat:@"%@元",_realityAmountValue];//实际到账金额
//                    
//                    
//                    
...
- (void)createMaskView{
    ...
    _footerView = [[UIView alloc]initWithFrame:CGRectMake(0, kScreenHeight, kScreenWidth, 1357*kPlusHeightScale)];
    _footerView.backgroundColor = kColorWhite;
    [_lastWindow addSubview:_footerView];
    
    UILabel *chooseCard = [[UILabel alloc]initWithFrame:CGRectMake(0, 0, kScreenWidth, 142/3)];
    chooseCard.textAlignment = NSTextAlignmentCenter;
    chooseCard.text = @"选择银行卡";
    chooseCard.font = Font(50/3);
    
    [_footerView addSubview:chooseCard];
    ...
    //线
    UILabel *lineGray = [UILabel new];
    lineGray.frame = CGRectMake(0, 142/3, kScreenWidth, 0.8);
    lineGray.backgroundColor =RGB(210, 210, 210, 1) ;
    [_footerView addSubview:lineGray];
    
//    [lineGray mas_makeConstraints:^(MASConstraintMaker *make) {
//        make.top.equalTo(chooseCard.mas_bottom);
//        make.left.equalTo(self.view);
//        make.right.equalTo(self.view);
...
//    }];
    ...
    UIButton *hiddenBtn = [[UIButton alloc]initWithFrame:CGRectMake(0, 0, 139*kPlusScale, 139*kPlusHeightScale)];
    hiddenBtn.adjustsImageWhenHighlighted = NO;
    [hiddenBtn setImage:[UIImage imageNamed:@"closefoot"] forState:UIControlStateNormal];
    
    [_footerView addSubview:hiddenBtn];
    
    [hiddenBtn addTarget:self action:@selector(hideFooterView) forControlEvents:UIControlEventTouchUpInside];

    _withdraw  = [[WithdrawView alloc]initWithFrame:CGRectMake(0, 142/3+0.8, __kScreenWidth__, 1357*kPlusHeightScale-142/3)style:UITableViewStylePlain];
   
    _withdraw.indexdelegate = self;
     [_footerView addSubview:_withdraw];
...
- (void)changeIndex:(NSUInteger)index{
    ...
    WithdrawModel *model = self.data[index];
    
    
                _cardNumber = [NSString stringWithFormat:@"%@",model.card];
                NSString *cardNumb = [_cardNumber substringFromIndex:_cardNumber.length-4];
                _bankCardNumber = [NSString stringWithFormat:@"%@",model.bankcard];
    
                [_bankLogo sd_setImageWithURL:[NSURL URLWithString:model.logo]];
                _bankName.text = [NSString stringWithFormat:@"%@(尾号%@)",model.bankname,cardNumb];
     ...
     *银行卡名字
     */
    _bankNameValue =[NSString stringWithFormat:@"%@",model.bankname];
...
- (void)promptFee:(NSString *)string{
    ...
    CGFloat strWidth = [ self widthForString:string
                                    fontSize:14
                                   andHeight:14];
    promptLabel.textColor = kColorMain;
    promptLabel.text = string;
...
//
//    [_withdrawFee mas_makeConstraints:^(MASConstraintMaker *make) {
//        make.left.equalTo(withdrawFeeLabel.mas_right).offset(0.018*kScreenWidth);
//        make.right.equalTo(self.view).offset(-0.056*kScreenWidth);
//        make.top.equalTo(withdrawalAmountView.mas_bottom).offset(0.030*kScreenHeight);
//        make.height.mas_equalTo(14);
//        
//        
//    }];
