...
- (NSString *)description {
    ...
    while ((attributeName = [keyEnum nextObject])) {
        ...
        if ([self respondsToSelector:getSel]) {
            ...
            //            ITTDINFO(@"attributeName %@ value %@", attributeName,
            //            valueObj);
            if (valueObj) {
    ...
    if (customDesc && [customDesc length] > 0) {
        desc = [NSString
                stringWithFormat:@"%@:{%@,%@}", [self class], attrsDesc, customDesc];
    } else {
...
- (void)setAttributes:(NSDictionary *)dataDic {
    ...
    if (attrMapDic == nil) {
        ...
        [NSMutableDictionary dictionaryWithCapacity:[dataDic count]];
...
- (NSString *)cleanString:(NSString *)str {
    ...
    [cleanString replaceOccurrencesOfString:@"\r"
                                 withString:@""
                                    options:NSCaseInsensitiveSearch
                                      range:NSMakeRange(0, [cleanString length])];
    return cleanString;
