...
#import "UIViewExt.h"
#import "Masonry.h"
@implementation ExperienceCouponCell
...
- (void)setModel:(ExprienceModel *)model{
    _model = model;
...
//state判断状态，0为未使用，1位使用，2为过期
   if ([_state  isEqualToString:@"0"]) {
       ...
       _bgView.image = [UIImage imageNamed:@"redPacketusedBg1"];
   }else if ([_state isEqualToString:@"2"]){
       
       _nameLabel.textColor = RGB(235, 235, 235, 1);
  
  _bgView.image = [UIImage imageNamed:@"redPacketExpiredBg1"];
