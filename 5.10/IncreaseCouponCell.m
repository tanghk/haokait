...
#import "ExperienceCouponCell.h"
#import "UICommons.h"
#import "UIViewExt.h"
#import "Masonry.h"

@implementation IncreaseCouponCell
...
- (void)setModel:(IncreaseCouponModel *)model{
    _model = model;
    
    _parLabel.text =[NSString stringWithFormat:@"%@",_model.increase];
    _state = [NSString stringWithFormat:@"%@",_model.state];
    _descriotionDetail.text =[NSString stringWithFormat:@"%@",_model.descriptionDetail];
    //state判断状态，0为未使用，1位使用，2为过期
    if ([_state  isEqualToString:@"0"]) {
        _bgView.image = [UIImage imageNamed:@"redPacketRedBg3"];
     
        _descriotionDetail.textColor = RGB(255, 241, 176, 1);
        
    }else if ([_state isEqualToString:@"1"]){
           _bgView.image = [UIImage imageNamed:@"redPacketusedBg3"];
       
         _descriotionDetail.textColor = RGB(211, 211, 211, 1);
        
        
    }else if ([_state isEqualToString:@"2"]){
           _bgView.image = [UIImage imageNamed:@"redPacketExpiredBg3"];
                _descriotionDetail.textColor = RGB(235, 235, 235, 1);
...
- (void)createCouponsContentView{
     ...
     *name
     */
    _descriotionDetail = [[UILabel alloc]init];
     _descriotionDetail.font = Font(13);
    [self.contentView addSubview:_descriotionDetail];
    
    [_descriotionDetail mas_makeConstraints:^(MASConstraintMaker *make) {
    ...
    UILabel *increaseText = [[UILabel alloc]init];
    
    increaseText.font = [UIFont systemFontOfSize:34/2];
    increaseText.textColor = kColorWhite;
    increaseText.text = @"加息";
    
    [_bgView addSubview:increaseText];
    
    
    [increaseText mas_makeConstraints:^(MASConstraintMaker *make) {
